...
- `components/<course>/`: course-specific explorables and figures, registered in that course's `components/<course>/index.ts` (spread into `lib/mdx-components.tsx`, so parallel builders never edit the same registry). Component names are global across courses; pick collision-proof names. CNDC labs: DelayLab, QueueingLab, SwitchingLab, EncapsulationExplorer, ThroughputLab, SharedMediumLab, StatMuxStrip, AccessTechDiagram. CNDC static SVG figures: InternetMap, ProtocolExchange, MediaChart, LayerStacks, HopDiagram, MultiplexingDiagram, CaravanDiagram, IntensityCurve, BottleneckPipes, TracerouteMap. Every figure uses `Figure` and the `P` palette from `components/learning/Figure.tsx` (viewBox width 640, mono labels 9 to 12 px, blue = highlighted path, amber = shared or waiting, green = dedicated or ok, red = loss). Register new ones in `components/<course>/index.ts`. Screenshot every new figure in the browser before committing; label overlaps do not show up in the build.
...
| Slug | Course | Status |
|------|--------|--------|
| `cndc` | CSC 3205 Data Communication and Computer Networks (Dr. Maria Zuraiz; instructor slides call it DCCN) | Outline in hand. Weeks 1 to 4 built. Lectures 01 to 10 uploaded (lecture 05 missing). Dr. Zuraiz teaches from Rana Faisal Hayat's slides, so his name on a deck is expected, not a different instructor. Assignment 1 (due 2026-09-28) guidance is in the week-2 checkpoint. Quiz 1 on 2026-09-29, scope assumed lectures 01 to 04 (`courses/cndc/quizzes/quiz-1-notes.md`), revision lesson `week-02/quiz-1-revision`. |
...
Outlines for the courses still missing one are expected around the week of 2026-09-28. Build only courses that have an outline or an explicit go-ahead. The slugs above are proposals; if the owner drops materials under a different `courses/<slug>/`, use that.
...
- Each course: 4 quizzes and 4 assignments, 2 of each before the midterm (week 8) and 2 after. No fixed dates; the instructor announces them. CNDC quiz 1 is 2026-09-29 (week 3); owner expects quiz 2 around weeks 5 to 6. Web Tech quiz 1 is 2026-09-28.
